...
import datetime
from simoji.lib.BasicFunctions import find_nearest, savgol_smooth, interpol, project_2d_array_onto_grid
...
class AngleSpectrumReader:
    ...
    def _eval_sweepme_files(self, path: Path) -> (np.array, np.array, np.array, np.array):
        ...
        def read_motor_file(file_path: str) -> (float, float):
            ...
            time_tags = ["Time", "Time elapsed"]    # This is auto generated by SweepMe so it shouldn't change to often
            ...
            for time_tag in time_tags:
                if time_tag in lines[0]:
                    time_idx = lines[0].index(time_tag)
                    break
    ...
    def _execute_fit(self, init_para, steps, maxima, fit_function):
        ...
        for i in np.arange(len(init_para)):
            if not fit_valid:
                try:
                    popt, pcov = curve_fit(fit_function, xdata, ydata, p0=init_para[i])  # sigma=y_err
                except:
                    pcov = 'inf'
                    popt = [0]
