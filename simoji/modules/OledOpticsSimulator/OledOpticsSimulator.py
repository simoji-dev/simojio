...
from simoji.shared_resources.scripts.MaterialFileReader import MaterialFileReader

from simoji.shared_resources.scripts.TransferMatrix import TransferMatrix
from simoji.shared_resources.scripts.Polarization import Polarization
...
class OledOpticsSimulator(Simulator):
    """
    ...
    material_par = FileFromPathParameter(name="material",
                                              path=os.path.join("shared_resources", "data", "optical_constants"),
    ...
    pl_spectrum_par = FileFromPathParameter(name="PL spectrum",
                                                 path=os.path.join("shared_resources", "data", "PLSpectra"),
                                                 extension_list=[".txt"],
                                                 description="Photoluminescent emission spectrum")

    module_parameters.add_layer_parameters(parameters=[material_par], layer_type=LayerType.SEMI)
    module_parameters.add_layer_parameters(parameters=[material_par, thickness_par],
                                                layer_type=LayerType.SUBSTRATE)
    module_parameters.add_layer_parameters(parameters=[material_par, thickness_par],
                                                layer_type=LayerType.COHERENT)
    module_parameters.add_layer_parameters(
        parameters=[material_par, thickness_par, nb_dipoles_par, aniso_par,
                    gamma_par, eta_rad_par, pl_spectrum_par],
        layer_type=LayerType.EMISSION)
    ...
    module_parameters.add_generic_parameters(
        [polarization_par, wavelengths_par, wavevectors_par, angles_par,
         plot_mode_par, calc_powdiss_flag_par, calc_sri_flag_par,
         calc_eqe_flag_par, calc_loss_channels_flag_par])

    def __init__(self):
        super().__init__()

        # layer (property) lists
        self.layer_parameters_list = []
    ...
    def configure_generic_parameters(self, generic_parameters: dict):
        ...
        # get execution flags
        if self.calc_powdiss_flag_par.name in generic_parameters:
            self.calc_powdiss_flag = generic_parameters[self.calc_powdiss_flag_par.name]
        if self.calc_sri_flag_par.name in generic_parameters:
            self.calc_sri_flag = generic_parameters[self.calc_sri_flag_par.name]
        if self.calc_eqe_flag_par.name in generic_parameters:
            self.calc_eqe_flag = generic_parameters[self.calc_eqe_flag_par.name]
        if self.calc_loss_channels_flag_par.name in generic_parameters:
            self.calc_loss_channels_flag = generic_parameters[self.calc_loss_channels_flag_par.name]

        self.calc_u_mode = any((self.calc_powdiss_flag, self.calc_eqe_flag, self.calc_loss_channels_flag))
        if self.plot_mode_par.name in generic_parameters:
            self.plot_mode_powdiss = PlotMode(generic_parameters[self.plot_mode_par.name])

        # get list of polarization values which need to be calculated (convert 'total' to ['s', 'p'])
        if self.polarization_par.name in generic_parameters:
            polarization_str = generic_parameters[self.polarization_par.name]
        ...
        if self.wavelengths_par.name in generic_parameters:
            self.wavelength_arr = np.array(start_stop_step_to_list(generic_parameters[self.wavelengths_par.name]))

        # get normalized in-plane wave vector + replace value one (pole in calculation)
        if self.wavevectors_par.name in generic_parameters:
            u_real = start_stop_step_to_list(generic_parameters[self.wavevectors_par.name])
            try:
        ...
        if self.calc_sri_flag:
            if self.angles_par.name in generic_parameters:
                self.angles_deg = np.array(start_stop_step_to_list(generic_parameters[self.angles_par.name]))
                self.angles = self.angles_deg * np.pi / 180.

    def configure_layers(self, layer_parameters_list: list, layer_type_list: List[LayerType]):

        # layer type dependent properties
        self.layer_type_list = layer_type_list
        self.emission_layer_index_list = [idx for idx in range(len(self.layer_type_list))
                                          if (self.layer_type_list[idx] is LayerType.EMISSION)]
        self.coherent_layers_index_arr = np.array([idx for idx in range(len(self.layer_type_list))
                                                   if (self.layer_type_list[idx] in [LayerType.COHERENT,
                                                                                     LayerType.EMISSION])])
        self.is_coherent_list = [(self.layer_type_list[idx] in [LayerType.COHERENT, LayerType.EMISSION])
                                 for idx in range(len(self.layer_type_list))]

        if len(self.emission_layer_index_list) == 0:
            raise ValueError("No emission layer defined!")

        # layer parameter dependent properties
        self.get_layer_thickness_arr(layer_parameters_list)
        self.get_optical_constants_arr(layer_parameters_list)
        self.get_normalized_pl_spectra_dict(layer_parameters_list)

        # self.layer_parameters_list = layer_parameters_list
        for idx, layer_dict in enumerate(layer_parameters_list):
            for key in layer_dict:
                try:
                    self.layer_parameters_list[idx][key] = layer_dict[key]
                except:
                    self.layer_parameters_list.append({key: layer_dict[key]})
    ...
    def run_simulator(self):
        ...
        for emission_layer_idx in self.emission_layer_index_list:
            ...
            # todo: New module 'CalcEmissionProfile' -> load as sub-module
            #  -> 3 cases:  i) from absorption profile -> need excitation wavelength
            #               ii) from drift-diffusion -> new module
            #               iii) Define Gauss (position + width)
            #  New parameter type MultiSwitchPar (combo instead bool + switch_par_list for each combo value)
    ...
    def get_layer_thickness_arr(self, layer_parameters_list: list):

        for idx, layer_dict in enumerate(layer_parameters_list):
            if self.thickness_par.name in layer_dict:
                try:
                    self.layer_thickness_list[idx] = layer_dict[self.thickness_par.name]
                except:
                    self.layer_thickness_list.append(layer_dict[self.thickness_par.name])

        # check that first and last layer (semi layers) are None (they are not given in layer_parameters_list)
        if self.layer_thickness_list[0] is not None:
            self.layer_thickness_list = [None] + self.layer_thickness_list
        if self.layer_thickness_list[-1] is not None:
            self.layer_thickness_list += [None]

    def get_optical_constants_arr(self, layer_parameters_list: list):
        mat_file_reader = MaterialFileReader()

        # optical_constants_list = []
        for idx, layer_dict in enumerate(layer_parameters_list):

            if self.material_par.name in layer_dict:
                # read optical constants from file
                material_file = os.path.join(self.material_par.path, layer_dict[self.material_par.name])
                ...
                # optical_constants_list.append(np.array(nk_complex))

    def get_normalized_pl_spectra_dict(self, layer_parameters_list):
        ...
        for emission_layer_idx in self.emission_layer_index_list:
            layer_par_dict = layer_parameters_list[emission_layer_idx]
            if self.pl_spectrum_par.name in layer_par_dict:
                pl_spectrum_file = layer_par_dict[self.pl_spectrum_par.name]
    ...
    def calc_Ks_and_efficiencies(self, emission_layer_idx: int) -> (np.array, np.array, float, list):
        ...
        anisotropy_coefficient = self.layer_parameters_list[emission_layer_idx][self.aniso_par.name]
        gamma = self.layer_parameters_list[emission_layer_idx][self.gamma_par.name]
        eta_rad = self.layer_parameters_list[emission_layer_idx][self.eta_rad_par.name]
    ...
    def get_dipole_distribution(self, emission_layer_idx) -> (np.array, np.array):
        ...
        nb_dipoles = int(self.layer_parameters_list[emission_layer_idx][self.nb_dipoles_par.name])
    ...
    def plot_K_eml(self):
        ...
        # save data (contourf cannot be saved automtically by simoji)
    ...
    def normalize_sri(self):

        idx, value = find_nearest(list(self.angles), 0.)
        spectrum = self.sri.T[idx]
        self.sri = self.sri / max(spectrum)
    ...
    def do_for_each_variation_step(self, variables_list: list, current_variable_values: list,
                                   variation_items_list=None) -> list:
        """
        If the module is repeatedly executed for different variable values (variation option enable), this method is
        executed after the execution of the run method for each variation step.
        :return:
        """

        if self.calc_loss_channels_flag:
            if len(variables_list) == 1:  # otherwise it's difficult to plot (maybe nd plot with sliders?)

                var_value = current_variable_values[0]

                if variation_items_list is None:
                    # 1st iteration -> plot container needs to be initialized
                    plot_name = "optical loss channels"
                    var_name = variables_list[0].name

                    data_set_labels = self.loss_channels_labels
                    axes_labels = [[var_name, "loss channel efficiencies [%]"]] * len(self.loss_channels_labels)
                    data_ls = ['.-'] * len(self.loss_channels_labels)
                    suffix = 'optical_loss_channels'

                    added_loss_channels = self._add_loss_channels_for_plotting(self.loss_channels_list)
                    plot_data = [[[var_value], [added_loss_channels[i]]] for i in range(len(added_loss_channels))]

                    # variation_plot_container = PlotDataContainer(plot_name=plot_name,
                    #                                              plot_type=PlotType.ONE_DIMENSIONAL,
                    #                                              plot_data=plot_data,
                    #                                              axes_labels=axes_labels,
                    #                                              data_set_labels=data_set_labels,
                    #                                              data_ls=data_ls,
                    #                                              fill_between_1d_lines=True,
                    #                                              multi_data_sets=True,
                    #                                              suffix=suffix,
                    #                                              save=True)
                else:
                    variation_plot_container = variation_items_list[0]
                    previous_plot_data = variation_plot_container.plot_data  # [[var_value_list, loss_channel_list]]
                    var_value_list = previous_plot_data[0][0]
                    var_value_list.append(var_value)

                    current_loss_channel_list = self._add_loss_channels_for_plotting(self.loss_channels_list)

                    plot_data = []
                    for idx in range(len(previous_plot_data)):
                        loss_channel_list = previous_plot_data[idx][1] + [current_loss_channel_list[idx]]
                        plot_data.append([var_value_list, loss_channel_list])

                    variation_plot_container.update_plot_data(plot_data)

                # self.connector.send((ConnectorDataType.PLOT, variation_plot_container))
                print('todo: plot loss channels')
                return [variation_plot_container]

    def _add_loss_channels_for_plotting(self, loss_channel_list: list) -> list:
        added_loss_channels = []
        for idx in range(len(loss_channel_list)):
            if idx == 0:
                added_loss_channels = [loss_channel_list[0]]
            else:
                added_loss_channels.append(loss_channel_list[idx] + added_loss_channels[-1])
        return added_loss_channels

    def get_numerical_results_dict(self) -> dict:
        ...
        if self.calc_eqe_flag:
            ...
            if not self.eqe is None:
                eqe *= 100.
                numerical_results_dict.update({"EQE [%]": self.str_formatter.format(eqe)})
            else:
                numerical_results_dict.update({"EQE [%]": str(eqe)})

        if self.calc_loss_channels_flag:
            for idx, label in enumerate(self.loss_channels_labels):
                loss_channel_value = self.loss_channels_list[idx]
                if not self.loss_channels_list[idx] is None:
                    loss_channel_value *= 100.
                    numerical_results_dict.update({self.loss_channels_labels[idx] + ' [%]':
                                                       self.str_formatter.format(loss_channel_value)})
                else:
                    numerical_results_dict.update({self.loss_channels_labels[idx] + ' [%]': str(loss_channel_value)})

        return numerical_results_dict

    def get_optimization_dict(self) -> dict:
        optimization_dict = {
            "EQE":  self.eqe
        }
        for idx, label in enumerate(self.loss_channels_labels):
            loss_channel_value = self.loss_channels_list[idx]
            optimization_dict.update({label: loss_channel_value})
