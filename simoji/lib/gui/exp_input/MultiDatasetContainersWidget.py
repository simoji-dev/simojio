...
from simoji.lib.gui.exp_input.DatasetContainerWidget import DatasetContainerWidget

from typing import List, Callable
import copy


class MultiDatasetContainersWidget(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.window_title_prefix = "data_set_"
        ...
        self.add_context_menu_action(self.add_action, self._add_dataset_clicked)
    ...
    def _edit_parameter_container_list(self, parameter_container_list: List[ParameterContainer]) -> List[
        ParameterContainer]:
        return parameter_container_list

    def _add_dockwidget(self, parameter_container: ParameterContainer):

        parameter_container_widget = DatasetContainerWidget()
        parameter_container_widget.set_parameter_container(parameter_container)
        parameter_container_widget.delete_dataset_sig.connect(self._delete_container)
        parameter_container_widget.add_dataset_sig.connect(self._add_dataset_clicked)
        parameter_container_widget.float_par_added_sig.connect(self.float_par_added)
        parameter_container_widget.duplicate_dataset_sig.connect(self._duplicate_container)
    ...
    def _get_dockwidget(self, title: str, widget: QtWidgets.QWidget, movable=True) -> QtWidgets.QDockWidget:
        ...
        dockWidget.topLevelChanged.connect(self.rename_datasets)
    ...
    def _delete_container(self, parameter_container_widget: DatasetContainerWidget):
    ...
    def _add_dataset_clicked(self):
        parameter_container = ParameterContainer(ParameterCategory.DATASET)
        parameter_container.set_module(self._current_module)
        self.add_parameter_container(parameter_container)
        self.set_module(self._current_module)
